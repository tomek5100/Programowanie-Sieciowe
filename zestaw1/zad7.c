...
{
    int filedes1;
    if ((filedes1 = open("tekst2.txt", O_RDONLY, 0644)) == -1)
    {
        perror("Open1 error\n");
        exit(EXIT_FAILURE);
    }
    ...
    int filedes2;
    if ((filedes2 = open("kopia2.txt", O_WRONLY | O_CREAT | O_TRUNC, 0644)) == -1)
    {
        perror("Open2 error\n");
        exit(EXIT_FAILURE);
    }
    ...
    {
        // sprawdzam czy w buforze wystepuje znak nowej linii
        int even = 0;
        int iter = 0;
        for (int i = 0; i < readed; i++)
        {
            ...
            if (bufor[i] == '\n')
            {
                if ((even % 2) == 0)
                {
                    if (write(filedes2, bufor2, iter) == -1)
                    {
                        perror("Write error\n");
                        exit(EXIT_FAILURE);
                    }
                }
                memset(bufor2, '\0', iter); // czyscimy bufor
                iter = 0;
                even++;
            }
        }
